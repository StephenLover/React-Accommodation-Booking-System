...
__table = ["userInfo,csv", "accommodationInfo.csv",
            "trasaction.csv", "watchingList.csv",
            "pendingList.csv", "review.csv"]
readFilePath = './data/'
...
def gene_first_name(userInfoLen):
    first_name = pd.read_csv(readFilePath + 'first_name.csv')
...
def gene_last_name(userInfoLen):
    last_name = pd.read_csv(readFilePath + 'last_name.csv')
...
def save_to_file(data):
    for k, v in data.items():
        ...
        df = df[keys[k]]
        df.to_csv(readFilePath + filename + '.csv', header=True, index=False)
        print('Finish..')
...
user_email = list()
with open(readFilePath + 'user_email.txt', 'r') as f:
